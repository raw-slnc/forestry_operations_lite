...
class DemBrowserDialog(QtWidgets.QDialog):
    ...
    def _scan(self):
        ...
        # ── 長野県 DEM（DEMモードのみ。砂防課はDCHMからDSM自動生成、林務部はDSMなし）──
        if self._mode == "dem" and self._canvas_overlaps_nagano():
            sec_nagano = QtWidgets.QListWidgetItem("── Nagano Prefecture open-data DEM (Nagano) ──")
...
class ForestryOperationsLiteDockWidget(QtWidgets.QWidget, FORM_CLASS):
    ...
    def changeEvent(self, event):
        """ネイティブの最大化/復元（タイトルバーの最大化ボタン・ダブルクリック）
        が起きたら、本体の Fullscreen チェックボックスを実状態へ追従させる。
        3つの入口（最大化ボタン・タイトルバーのダブルクリック・チェックボックス）
        が同じ1つのウィンドウ状態を指すようにする。"""
        if event.type() == QEvent.Type.WindowStateChange:
            cb = getattr(self, "chkFullscreen", None)
            if cb is not None:
                with QSignalBlocker(cb):
                    cb.setChecked(self.isMaximized())
        super().changeEvent(event)
    ...
    def _ensure_alt_tab_visible(self):
        ...
        再表示時にQt/OS側で属性が戻ることがあるため、表示のたびに再適用する。"""
    ...
    def _build_extended_ui(self):
        ...
        self.btnTerrainToggle.setFocusPolicy(Qt.FocusPolicy.NoFocus)
        ...
        self.chkLoadIntegrated = QtWidgets.QPushButton("Risk Off")
    ...
    def _build_terrain_tab(self, parent):
        ...
        self._skip_dissolve_high_risk_runs = set()
        ...
        self._flow_buffer_state_tc = "off"  # Time load (tc) 専用 off/weak/strong
        ...
        self._orig_base_opacity = {}       # layer_id → 元 opacity（Layer Settings 選択レイヤーの復元用）
        ...
        self.spinPhiDeg.setRange(0, 60); self.spinPhiDeg.setValue(30); self.spinPhiDeg.setSuffix("°")
        self.spinCKpa   = QtWidgets.QDoubleSpinBox()
        self.spinCKpa.setRange(0, 200);  self.spinCKpa.setValue(3);  self.spinCKpa.setSuffix(" kPa")
        self.spinZm     = QtWidgets.QDoubleSpinBox()
        self.spinZm.setRange(0.1, 10);   self.spinZm.setValue(0.5); self.spinZm.setSuffix(" m")
        ...
        self.spinFsThresh.setRange(0.5, 3.0); self.spinFsThresh.setValue(1.2); self.spinFsThresh.setSingleStep(0.1)
        _ps_defs = [
    ...
    def apply_layer_display(self):
        ...
        _cur_ids = {lyr.id() for lyr in (bg, tile, gpkg) if lyr is not None}
        for _lid in list(self._orig_base_opacity):
            if _lid in _cur_ids:
                continue
            _l = proj.mapLayer(_lid)
            if _l is not None:
                _l.setOpacity(self._orig_base_opacity[_lid])
                _l.triggerRepaint()
            del self._orig_base_opacity[_lid]
        for _l, _spin in ((bg, self.spinBgOpacity),
                          (tile, self.spinTileOpacity),
                          (gpkg, self.spinGpkgOpacity)):
            if _l is None:
                continue
    ...
    def _restore_base_layer_opacity(self):
        """Layer Settings で触った実レイヤーの不透明度を元値へ戻す。"""
        proj = QgsProject.instance()
        for _lid, _op in list(self._orig_base_opacity.items()):
            _l = proj.mapLayer(_lid)
            if _l is not None:
                _l.setOpacity(_op)
                _l.triggerRepaint()
        self._orig_base_opacity.clear()
    ...
    def _apply_raster_color(lyr, base_name, filter_mode="off"):
        ...
        FIXED = {
            ...
            "integrated_high_risk_mask": [
                (0.0, QColor(200,   0,   0,   0), ""),
                (1.0, QColor(200,   0,   0, 160), "High Risk Areas"),
            ],
        }
        ...
        if vmin >= vmax and base_name not in FIXED:
            return
    ...
    # 解析結果グループ内の順序: 値が大きいほど上（描画前面）
    # 下から: elev_relief → hillshade → Slope Stability → Overall Risk
    #        → Valley → Wetland → Flow（最前面）
    # 背景地形（elev_relief/hillshade）は同じグループの中で常に最下位＝背景。
    _KEY_RANK = {
        "elev_relief": -2, "hillshade": -1,
        "stability": 0, "integrated": 1, "valley": 2, "wetland": 3, "flow": 4,
    }
    ...
    def _terrain_button_label(self, key):
        if key == "integrated":
            st = self._terrain_cycle_state.get("integrated", -1)
            return ("Overall Risk" if st == 0
                    else "High Risk" if st == 1 else "Risk Off")
    ...
    def _terrain_display_patterns(self, key, out_dir, analysis_number,
        ...
        integrated の2段目は軽量なラスタマスクを優先し、未生成の既存解析では
        dissolved GPKG へフォールバックする。
        """
        if key != "integrated":
        ...
        high_mask_path = os.path.join(
            out_dir, analysis_number, "integrated_high_risk_mask.tif")
        if os.path.exists(high_mask_path):
            high_risk_pattern = (
                "integrated_high_risk_mask", "High Risk Areas", "raster", ".tif")
        else:
            high_risk_base = self._ensure_dissolved_high_risk(out_dir, analysis_number)
            high_risk_pattern = (
                "integrated_high_risk", "High Risk Areas", "vector", ".gpkg",
                high_risk_base)
        return [high_risk_pattern]

    def _confirm_high_risk_dissolve(self, run_key):
        """既存 High Risk Areas の表示軽量化変換を実行するか確認する。"""
        dialog = QtWidgets.QDialog(self)
        dialog.setWindowTitle("High Risk Areas")
        ...
        title = QtWidgets.QLabel("Convert for faster display")
        font = title.font()
        font.setBold(True)
        title.setFont(font)
        ...
        message = QtWidgets.QLabel(
            "Existing High Risk Areas will be converted to a dissolved GPKG "
            "before display."
        )
        message.setWordWrap(True)
        layout.addWidget(message)

        action = {"convert": False, "skip": False}
        yes_btn = QtWidgets.QPushButton("Convert and Show")
        no_btn = QtWidgets.QPushButton("Skip This Time")
        remember_chk = QtWidgets.QCheckBox("Do not ask again this session")
        ...
        yes_btn.clicked.connect(accept_convert)
        no_btn.clicked.connect(reject_skip)

        buttons = QtWidgets.QHBoxLayout()
        buttons.addWidget(yes_btn)
        buttons.addWidget(no_btn)
        buttons.addStretch(1)
        buttons.addWidget(remember_chk)
        ...
        if action["convert"]:
            return True
        if action["skip"] and remember_chk.isChecked():
            self._skip_dissolve_high_risk_runs.add(run_key)
        return False

    def _ensure_dissolved_high_risk(self, out_dir, analysis_number):
        """既存 High Risk Areas の表示用 dissolved GPKG 名を返す。"""
        run_dir = os.path.join(out_dir, analysis_number)
        run_key = os.path.normcase(os.path.abspath(run_dir))
        src = os.path.join(run_dir, "integrated_high_risk.gpkg")
        dst = os.path.join(run_dir, "integrated_high_risk_dissolved.gpkg")
        if not os.path.exists(src):
            return "integrated_high_risk"
        try:
            if (os.path.exists(dst) and
                    os.path.getmtime(dst) >= os.path.getmtime(src)):
                return "integrated_high_risk_dissolved"
            if run_key in self._skip_dissolve_high_risk_runs:
                return "integrated_high_risk"
            self._zoom_preview_to_analysis_extent_if_available()
            QtWidgets.QApplication.processEvents()
            if not self._confirm_high_risk_dissolve(run_key):
                return "integrated_high_risk"
            self.lblLoadStatus.setText("Preparing High Risk Areas...")
            QtWidgets.QApplication.processEvents()
            from .terrain import result_writer as rw
            dissolved = rw.dissolve_gpkg(
                src, run_dir, "integrated_high_risk_dissolved", overwrite=True)
            if dissolved and os.path.exists(dissolved):
                return "integrated_high_risk_dissolved"
        except Exception as exc:
            self.lblLoadStatus.setText(
                f"High Risk dissolve failed; showing original: {exc}")
        return "integrated_high_risk"

    def _cycle_terrain_layer(self, key, desired_state=None):
        ...
        if key == "integrated":
            # High Risk 側のファイルが無い解析では state 1 を飛ばして OFF へ
            if next_state != -1 and not available:
                next_state = -1
        else:
            if not available:
        ...
        # 背景地形（hillshade/elev_relief）は _insert_terrain_layer_ordered が
    ...
    def _toggle_terrain_layer(self, key, checked):
        ...
        if checked:
            ...
            added = []
            ids = []
            for pat in self._terrain_display_patterns(key, out_dir, analysis_number):
                color_name, label, kind, ext = pat[0], pat[1], pat[2], pat[3]
                file_base = pat[4] if len(pat) > 4 else color_name
                path = os.path.join(out_dir, analysis_number, f"{file_base}{ext}")
    ...
    def _clear_dem_selection(self, clear_dsm=False, dem_info="Not set"):
        self._dem_path = ""
        self._dem_actual_path = ""
        self._cs_map_dem_path = ""
        ...
        self.lblDemInfo.setText(dem_info)
    ...
    def _on_browse_dem(self):
        ...
        if self._dem_path:
            # クリア
            _clear_dsm = self._dem_path in (
                DemBrowserDialog.VS_LP_GRID_SENTINEL,
                DemBrowserDialog.NAGANO_SABO_DEM_SENTINEL,
                DemBrowserDialog.NAGANO_RINMU_DEM_SENTINEL,
            )
            # VS LP/Grid・長野砂防DEMはDEM取得に紐づくDSMを連動クリアする。
            # 長野林務部DEMはDSMなしでDSM Browseを無効化しているため状態を戻す。
            self._clear_dem_selection(clear_dsm=_clear_dsm)
            return
        initial_dir = ""
        ...
        if dlg.exec() == QtWidgets.QDialog.DialogCode.Accepted:
            ...
            if path == DemBrowserDialog.VS_LP_GRID_SENTINEL:
                ...
                if not self._load_vs_lp_grid():  # auto_dsm=True (default)
                    self._clear_dem_selection(
                        clear_dsm=True,
                        dem_info=self.lblDemInfo.text(),
                    )
                ...
                if not self._load_nagano_sabo_dem():  # auto_dsm=True (default)
                    self._clear_dem_selection(
                        clear_dsm=True,
                        dem_info=self.lblDemInfo.text(),
                    )
                ...
                if not self._load_nagano_rinmu_dem():
                    self._clear_dem_selection(
                        clear_dsm=True,
                        dem_info=self.lblDemInfo.text(),
                    )
    ...
    def _load_vs_lp_grid(self, auto_dsm=True, update_cs_map_source=True, confirm=True):
        ...
        if confirm:
            reply = QtWidgets.QMessageBox.question(
                self,
                "Virtual Shizuoka DEM Fetch",
                f"This will fetch {len(codes)} VS LP/Grid tile(s) over the network.\n"
                "DSM creation can also fetch large point-cloud data after DEM loading.\n\n"
                "Continue?",
    ...
    def _load_nagano_sabo_dem(self, update_cs_map_source=True, auto_dsm=True, confirm=True):
        ...
        try:
            ...
            if auto_dsm and not self._dem_load_cancel:
                ...
                self.btnBrowseDsm.setText("")
                dsm_result = self._compute_nagano_sabo_dsm(codes, tif_path)
                if dsm_result == "cancel":
                    self._clear_dem_selection(clear_dsm=True, dem_info="Cancelled")
                    return False
                if not getattr(self, "_dsm_loader", None):
                    self._dsm_path = ""
                    self.txtDsmPath.clear()
    ...
    def _compute_nagano_sabo_dsm(self, codes, dem_tif_path):
        ...
        DEMと同じタイルコード群を使い、DCHMがDEM範囲を覆う場合のみDSMを作る。"""
        from .nagano_dchm import download_tile_tif as download_dchm_tile
        from .nagano_sabo import tile_bbox as nagano_tile_bbox
        from .vs_lp import merge_tifs
        from .terrain.dem_loader import DEMLoader
        from osgeo import gdal
        import numpy as _np
        ...
        def _covers(outer, inner, tol=0.5):
            return (
                outer[0] <= inner[0] + tol
        ...
        def _validate_dchm_tile(code, path):
            ds = gdal.Open(path)
            ...
            if band is None:
                ds = None
                return False, "has no raster band"
            # Read the corners lightly; this catches many truncated/corrupt GeoTIFF caches.
            if band.ReadRaster(0, 0, 1, 1) is None:
                ds = None
                return False, "cannot read raster data"
            if band.ReadRaster(ds.RasterXSize - 1, ds.RasterYSize - 1, 1, 1) is None:
                ds = None
                return False, "cannot read raster data"
            actual = _bounds_from_ds(ds)
            ds = None
            expected = nagano_tile_bbox(code)
            if not _covers(actual, expected):
                return False, "does not cover the expected tile extent"
            return True, ""

        def _show_dchm_problem_dialog(details):
            msg = QtWidgets.QMessageBox(self)
            icon_enum = getattr(QtWidgets.QMessageBox, "Icon", QtWidgets.QMessageBox)
            msg.setIcon(icon_enum.Warning)
            msg.setWindowTitle("DCHM Download Problem")
            msg.setText("Some DCHM tiles could not be downloaded or appear to be incomplete.")
            msg.setInformativeText(
                "DSM cannot be created reliably for the selected DEM extent.\n"
                "Please try downloading again, or check the source server directly "
                "to confirm that the required tiles are available."
            )
            if details:
                msg.setDetailedText(details)
            button_role = getattr(QtWidgets.QMessageBox, "ButtonRole", QtWidgets.QMessageBox)
            retry_btn = msg.addButton("Retry", button_role.AcceptRole)
            continue_btn = msg.addButton("Continue without DSM", button_role.DestructiveRole)
            cancel_btn = msg.addButton("Cancel", button_role.RejectRole)
        ...
        while True:
            ...
            for i, code in enumerate(codes):
                ...
                if not path:
                    missing.append(code)
                    continue
                ok, reason = _validate_dchm_tile(code, path)
                if ok:
                    dchm_paths.append(path)
                else:
                    invalid.append(f"{code}: {reason}")
                    invalid_paths.append(path)

            if len(dchm_paths) == len(codes):
                break

            detail_lines = []
            if missing:
                detail_lines.append(
                    "Missing DCHM tiles: " + ", ".join(missing[:20])
                    + (" ..." if len(missing) > 20 else "")
                )
            if invalid:
                detail_lines.append(
                    "Incomplete or invalid DCHM tiles: " + "; ".join(invalid[:20])
                    + (" ..." if len(invalid) > 20 else "")
                )
            action = _show_dchm_problem_dialog("\n".join(detail_lines))
            if action == "retry":
                for path in invalid_paths:
                    try:
                        ...
                        pass
                self.lblDsmInfo.setText("Retrying DCHM download...")
                QtWidgets.QApplication.processEvents()
                continue
            if action == "continue":
                self.lblDsmInfo.setText("Skipped — no DSM")
        ...
        try:
            ...
            if dem_ds is None:
                raise RuntimeError("DEM cannot be opened")
            dem_gt = dem_ds.GetGeoTransform()
            dem_wkt = dem_ds.GetProjection()
            w, h = dem_ds.RasterXSize, dem_ds.RasterYSize
            dem_bounds = _bounds_from_ds(dem_ds)
            dem_band = dem_ds.GetRasterBand(1)
            dem_nodata = dem_band.GetNoDataValue()
            dem_arr = dem_band.ReadAsArray().astype(_np.float64)
            dem_ds = None

            merged_ds = gdal.Open(dchm_merged)
            if merged_ds is None:
                raise RuntimeError("Merged DCHM cannot be opened")
            dchm_bounds = _bounds_from_ds(merged_ds)
            merged_ds = None
            if not _covers(dchm_bounds, dem_bounds):
                raise RuntimeError("Merged DCHM does not cover the DEM extent")
            ...
            if dchm_ds is None:
                raise RuntimeError("DCHM warp failed")
    ...
    def _zoom_preview_to_analysis_extent_if_available(self):
        ...
        if (not getattr(self, "_initializing", False)
                and not getattr(self, "_map_locked", False)
    ...
    def _unload_terrain_group(self, reset_controls=True):
        ...
        try:
            from osgeo import gdal as _gdal
            for _mp in self._flow_buffer_mem_paths:
                _gdal.Unlink(_mp)
        except Exception:  # nosec B110
            pass
        self._flow_buffer_mem_paths = []
    ...
    def _prepare_dem_for_current_extent(self, status_prefix):
        ...
        if dem_path_cur in tile_sentinels:
            ...
            if dem_path_cur == DemBrowserDialog.VS_LP_GRID_SENTINEL:
                self._load_vs_lp_grid(auto_dsm=False, update_cs_map_source=True, confirm=False)
            elif dem_path_cur == DemBrowserDialog.NAGANO_SABO_DEM_SENTINEL:
                self._load_nagano_sabo_dem(update_cs_map_source=True, auto_dsm=False, confirm=False)
            elif dem_path_cur == DemBrowserDialog.NAGANO_RINMU_DEM_SENTINEL:
                self._load_nagano_rinmu_dem(update_cs_map_source=True, confirm=False)
            else:
                self._load_gsi_dem(dem_path_cur, update_cs_map_source=True)
    ...
    def _prepare_dem_for_cs_map_export(self):
        """取得済み/選択済みDEM全体をCS MAP用に読み込む。

        CS Map Export はDEM取得を発生させない。取得範囲の不足確認と再取得は
        DEM選択時または解析開始時の責務に限定する。
        """
        tile_sentinels = (
            DemBrowserDialog.GSI_DEM1A_SENTINEL,
            DemBrowserDialog.GSI_DEM5A_SENTINEL,
    ...
    def _cs_map_dem_source_label(self):
        ...
        fixed_labels = {
            ...
            _S.NAGANO_SABO_DEM_SENTINEL: "Nagano DEM (Sabo Division, 2021-2025), 0.5m, source: geospatial.jp",
            _S.NAGANO_RINMU_DEM_SENTINEL: "Nagano DEM (Forestry Dept, 2013-2014), 0.5m, source: geospatial.jp",
        }
    ...
    def _on_export_clicked(self):
        ...
        style_fail = 0
        buffer_errors = []
        # プレビュー/ツリー/メインと同じ _KEY_RANK 降順で追加する。
        # addLayer/addGroup は末尾（＝下）に足すので、高ランクから順に
        # 追加すると上から Flow → … → Slope → 背景 の並びになる。
        for key in sorted(self._TERRAIN_PATTERNS,
            ...
            for pat in patterns:
                ...
                if key == "flow" and kind == "raster" and flow_buffer_state != "off":
                    ...
                    try:
                        _bqml = os.path.splitext(buffer_path)[0] + ".qml"
                        _bmsg, _bok = buffer_lyr.saveNamedStyle(_bqml)
                        if not _bok:
                            style_fail += 1
                    except Exception:
                        style_fail += 1
        ...
        if added == 0:
            ...
            if style_fail:
                msg += f" ({style_fail} style file(s) not written.)"
    ...
    def _run_terrain_analysis(self):
        ...
        _is_overwrite = self.chkOverwrite.isChecked()
        _retrying = getattr(self, "_overwrite_retried", False)
        # 実行前に地形レイヤ表示状態を退避（上書きクリーンアップで消える前）。
        # 新規実行では完了後に新解析へ引き継ぎ、上書きでは使わずクリアする。
        # 上書きロックの自動再実行（再入）時は退避済みなので取り直さない。
        if not _retrying:
            self._pre_run_display_state = self._capture_terrain_display_state()
        _pre_run_display_state = getattr(self, "_pre_run_display_state", {})

        # ── 解析面積チェック（プレビューキャンバス範囲） ──
        _limit_ha = self.cmbAreaLimit.currentData()
        if _limit_ha and _limit_ha > 0 and not _retrying:
            try:
                import math as _math
                _ext = self.preview_canvas.extent()
                _crs = self.preview_canvas.mapSettings().destinationCrs()
        ...
        # ── DEMタイルソース: 範囲外チェック→警告→OKなら再取得（DSMは連動）──
        # DEMが解析対象そのもの。範囲内なら既存データのまま解析続行し、範囲外の
        # 時だけ確認の上で再取得する。DSMは元々設定されていた場合のみ連動して
        # 一緒に再取得する（DSM単体では独立に範囲外チェックしない——DSMは常に
        # DEMと同じ範囲・同じタイミングで取得されるため、DEMが範囲内である限り
        # DSMも範囲内のはず）。
        ...
        if _dem_path_cur in _TILE_SENTINELS:
            _dem_outside = (
                getattr(self, "_terrain_loader", None) is None
                or self._canvas_outside_loader(self._terrain_loader)
            )
            if _dem_outside:
                reply = QtWidgets.QMessageBox.question(
                    self, "Analysis Range Confirmation",
                    "The analysis range extends beyond the current DEM coverage.\nRe-fetch?",
                ...
                if self._dem_path == DemBrowserDialog.VS_LP_GRID_SENTINEL:
                    _want_dsm = (_dsm_path_cur == DemBrowserDialog.VS_LP_GROUND_SENTINEL)
                    self._load_vs_lp_grid(auto_dsm=False, update_cs_map_source=True, confirm=False)
                    if _want_dsm:
                        self._load_vs_lp_ground()
                elif self._dem_path == DemBrowserDialog.NAGANO_SABO_DEM_SENTINEL:
                    _want_dsm = (_dsm_path_cur == DemBrowserDialog.NAGANO_SABO_DSM_SENTINEL)
                    self._load_nagano_sabo_dem(auto_dsm=_want_dsm, update_cs_map_source=True, confirm=False)
                elif self._dem_path == DemBrowserDialog.NAGANO_RINMU_DEM_SENTINEL:
                    self._load_nagano_rinmu_dem(update_cs_map_source=True, confirm=False)
                else:
                    self._load_gsi_dem(self._dem_path, update_cs_map_source=True)
        ...
        def _release_folder_layers(_folder):
            ...
            def _flush_provider_connections():
                ...
                # 期待する（ファイルパスではない）。保存済み GeoPackage 接続の
                # うち対象ファイルを指すものを名前で特定して破棄する。解析
                # 出力は通常ブラウザ未登録なので該当なし＝何もしないことが多い。
                # 引数なしの全体 invalidation は QGIS + Windows で provider 側
                # access violation を誘発するため呼ばない。
        ...
        # 上書き実行は「計算前」に旧フォルダを削除するため、対象が存在するときは
        # 削除前に確認する。キャンセルなら何もせず閉じる。自動再実行の再入時
        # （_retrying）は既に同意済みなので出さない。
        if _is_overwrite and not _retrying:
            import re as _re_ck
            _ov_targets = sorted(
                n for n in os.listdir(out_dir)
                if _re_ck.fullmatch(r'\d{4}(\+\d+)?', n) and n[:3] == seq
                and os.path.isdir(os.path.join(out_dir, n))
            )
            if _ov_targets:
                _reply = QtWidgets.QMessageBox.question(
                    self, "Overwrite analysis",
                    "The existing analysis "
                    f"({', '.join(_ov_targets)}) will be deleted before running.\n\n"
                    "Continue?",
                    QtWidgets.QMessageBox.StandardButton.Ok
                    | QtWidgets.QMessageBox.StandardButton.Cancel,
                    QtWidgets.QMessageBox.StandardButton.Cancel,
                ...
                if _reply != QtWidgets.QMessageBox.StandardButton.Ok:
                    self.lblAnalysisStatus.setVisible(True)
                    self.lblAnalysisStatus.setText("Overwrite cancelled.")
                    return
        ...
        if self.chkOverwrite.isChecked():
            ...
            # Windows では表示中の GeoPackage がロックされるため、上書き削除に
            # 入る前に全表示ボタンを OFF にし管理レイヤを外す。
            ...
            for name in os.listdir(out_dir):
                ...
                # 多い（SQLite/OGR は .gpkg を開くがフォルダは掴まない）。
            ...
            if _lock_err is not None:
                ...
                # する。再入側が "Resampling..." 等で lblAnalysisStatus を上書き
                # するので、それまで読める間（1.5秒）だけ遅らせる。
                if not _retrying:
        ...
        # ここまで来たら上書き前のロック処理は通過済み。自動リトライ時に
        # "still locked / Retrying..." を残すと、解析中もロック処理中に見える。
        self.lblAnalysisStatus.setVisible(False)
        self.lblAnalysisStatus.clear()
        ...
        try:
            ...
            if os.path.exists(tmp_folder):
                _shutil.rmtree(tmp_folder)
            # 上書き時は事前クリーンアップで旧レイヤーを消しているので、
            # ボタン表示を現実（何も表示なし）に合わせる。
            if _is_overwrite:
                self._unload_terrain_group(reset_controls=True)
            return
        except Exception as e:
            ...
            if _dsm is not None:
                _dsm.data = None
            # 上書きロックの自動リトライ枠をリセット（この実行が cleanup を
            # 通過したので、次の実行はまた1回だけ黙って再試行できる）。
            self._overwrite_retried = False
        ...
        # 実行後、地形レイヤ表示を新解析の現実へ同期する。
        # 上書き: 全クリア（レイヤー除去＋ボタン・内部状態リセット）＝「消した」を明示。
        # 新規: 直前の表示状態を新解析へ引き継ぐ（セレクタ切替と同じ挙動）。
        self._unload_terrain_group(reset_controls=True)
        if not _is_overwrite:
            self._create_terrain_group(analysis_number)
            self._restore_terrain_display_state(_pre_run_display_state)
    ...
    def _load_settings(self):
        ...
        # ── 解析パラメータ ──
        self.spinPhiDeg.setValue(    f("spin_phi_deg",     30.0))
        self.spinCKpa.setValue(      f("spin_c_kpa",        3.0))
        self.spinZm.setValue(        f("spin_zm",           0.5))
        self.spinMSat.setValue(      f("spin_m_sat",        0.5))
        self.spinFsThresh.setValue(  f("spin_fs_thresh",    1.2))
    ...
    def closeEvent(self, event):
        ...
        if self.iface is not None:
            try:
                ...
                pass
        self._restore_base_layer_opacity()
